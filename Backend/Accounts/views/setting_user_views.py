...
def password_setting(request, pk):
    ...
    if request.user == user:
        ...
        return Response({'error' : '유저 본인의 비밀번호만 변경할 수 있습니다.'}, status=status.HTTP_403_FORBIDDEN)


@api_view(['PUT'])
@authentication_classes([JSONWebTokenAuthentication])
@permission_classes([IsAuthenticated])
def pincode_setting(request, pk):
    user = get_object_or_404(User, pk=pk)
    if request.user == user:
        ...
        return Response({'error' : '유저 본인의 핀번호만 변경할 수 있습니다.'}, status=status.HTTP_403_FORBIDDEN)
...
def user_delete(request, pk):
    ...
    if request.user == user:
        ...
        return Response({'error' : '유저 본인만 탈퇴할 수 있습니다.'}, status=status.HTTP_403_FORBIDDEN)
