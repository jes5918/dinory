...
def diary_create(request):
    ...
    if Note.objects.filter(year=year, month=month, child=child).exists():
        note = get_object_or_404(Note, year=year, month=month, child=child)
        if os.path.isfile(note.img.path):
    ...
    if serializer.is_valid(raise_exception=True):
        ...
        t1 = Thread(target=sentence_create, args=(content_list, child, request.user,))
        t1.start()
        return Response(serializer.data, status=status.HTTP_201_CREATED)


def sentence_create(arr, child, user):
    for content in arr:
        if Sentence.objects.filter(child=child, content=content).exists():
            ...
            if sentence_serializer.is_valid(raise_exception=True):
                sentence_serializer.save(user=user, child=child)
        words = word_tokenize(content)
        words_list = check_word(words)
        words_list = nltk.pos_tag(words_list)
        for word in words_list:
            word_id = word_create(word, child, user)
...
def word_create(word, child, user):
    ...
    if Word.objects.filter(child=child, content=word_checked).exists():
        ...
        if word_serializer.is_valid(raise_exception=True):
            word_serializer.save(user=user, child=child)
            word_id = word_serializer.data['id']
    return word_id