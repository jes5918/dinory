...
- **다중 프로필 생성 기능** (아이가 2명 이상인 경우를 위해서)
  ...
  - 로그인 > 계정 선택 > 아이 사용 > 다른 계정 변경할 때 엄마 pin번호 입력해야지 타 계정으로 전환 가능.
...
- **작성 완료 후 스피킹, 리딩 파트로 넘어감 **(읽고 녹음만 가능한 페이지)
  ...
  - 해당 문장 녹음본이 발음 유사도 60% 이상이어야지만 넘어갈 수 있음
  - 일정 유사도가 나오지 않으면 캐릭터가 "다시해봐!!!" 라고 안내해줌
...
- **단어 저장은 단어, 뜻, 형태에 대한 충분한 정보가 같이 저장**
  ...
  _++ (추후 추가 기능) 단어장을 활용하여 단어 복습하는 기능 추가_
